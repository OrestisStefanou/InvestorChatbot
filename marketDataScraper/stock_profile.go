...
func GetStockProfile(symbol string) (StockProfile, error) {
	...
	if !ok || len(nodes) < 3 {
		return StockProfile{}, fmt.Errorf("unexpected structure in 'nodes'")
	}

	// Access the second element in "nodes" which contains the data we are interested in
	nodeData, ok := nodes[2].(map[string]interface{})
	if !ok {
		return StockProfile{}, fmt.Errorf("unexpected structure in 'nodes[2]'")
	}

	data, ok := nodeData["data"].([]interface{})
	if !ok {
		return StockProfile{}, fmt.Errorf("unexpected structure in 'data'")
	}

	dataMap, ok := data[0].(map[string]interface{})
	if !ok {
		return StockProfile{}, fmt.Errorf("unexpected structure in 'data[0]'")
	}

	descriptionIndex, ok := dataMap["description"].(float64)
	if !ok {
		return StockProfile{}, fmt.Errorf("unexpected structure for 'description'")
	}

	profileIndex, ok := dataMap["profile"].(float64)
	if !ok {
		return StockProfile{}, fmt.Errorf("unexpected structure for 'profile'")
	}
	...
	if !ok {
		return StockProfile{}, fmt.Errorf("unexpected structure for 'industry'")
	}
	...
	if !ok {
		return StockProfile{}, fmt.Errorf("unexpected structure for 'sector'")
	}
